public class GameLogic {
    ...
    public GameLogic(Board board, GameLoopListener listener) {
        this.board   = board;
        this.listener = listener; // Lưu lại listener để dùng cho hiệu ứng nổ
    ...
    public void lockPiece() {
        ...
        // 2. THÊM DÒNG NÀY để hiện hiệu ứng nổ (nếu class GameLogic này có quyền truy cập effectManager)
        if (clearedRows.length > 0) {
            for (int row : clearedRows) {
                if (listener != null) {
                    listener.getEffectManager().addExplosion(row - Board.HIDDEN_ROWS, Board.WIDTH);
                }
            }
        }
        spawnNextPiece();
