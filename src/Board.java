public class Board {
    ...
    public int[] clearLines() {
        ...
        for (int r = TOTAL_ROWS - 1; r >= 0; r--) {
            if (isRowFull(r)) {
                clearedList.add(r);
                // Sửa tên hàm thành removeRow cho khớp với bên dưới của bạn
                removeRow(r);      // Xóa hàng đó
                r++;
            }
        }
        int[] result = new int[clearedList.size()];

        for (int i = 0; i < clearedList.size(); i++) {
            // Copy từng giá trị từ List sang mảng int[]
            result[i] = clearedList.get(i);
        }
        return result;
    }
